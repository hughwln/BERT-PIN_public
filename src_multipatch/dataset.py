...
def get_random_mask(load_gt):
    dim_day = 24 * config.NUM_H
    n_day = int(config.DIM_INPUT / dim_day)
    mask = np.zeros(config.DIM_INPUT, dtype=bool)

    for i in range(n_day):
        rad = np.random.random()
        if rad < 0.5:
            continue
        load_day = load_gt[i * dim_day: (i + 1) * dim_day]
        if config.USE_CENTRAL_MASK:
            patch_start = dim_day // 2 - config.DIM_PATCH // 2
        else:
            peak_index = max(load_day.argmax(axis=0) - config.DIM_PATCH // 2, 1)
            patch_start = min(peak_index, dim_day - config.DIM_PATCH - 1)
        patch_start = i * dim_day + patch_start
        mask[patch_start:patch_start + config.DIM_PATCH] = True  # hole-True
...
if config.GEN_DATASET:
    ...
    if config.FEEDER == 'Fayetteville':
        ...
        raw_data = pd.read_csv('../data/raw_data/NewRiver/NonCVR_LT_2000_2000.csv', index_col=0)
